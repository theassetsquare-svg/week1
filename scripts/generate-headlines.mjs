...
function getTemplateSlots() {
  return [
    ...
    { mechanism:'Specificity', driver:'구체성', variants:[
      (n,v)=>`${n} 입장 전 체크 ${v.checklist.length}개 항목 상세 해설`,
      (n,v)=>`${n} 체크리스트 ${v.checklist.length}항목 전체 해설`,
      (n,v)=>`${n} 체크 ${v.checklist.length}개 빠짐없이 꼼꼼 해설`,
      (n,v)=>`${n} 입장 체크 ${v.checklist.length}가지 항목별 해설`,
    ]},
    { mechanism:'Specificity', driver:'구체성', variants:[
      (n,v)=>`${n} 피크타임 ${v.timeline.length}단계 시간대별 공략 리포트`,
      (n,v)=>`${n} ${v.timeline.length}구간 시간대별 즐기는 법 리포트`,
      (n,v)=>`${n} 타임라인 ${v.timeline.length}단계 활용 팁 리포트`,
      (n,v)=>`${n} 시간대 ${v.timeline.length}구간 분위기별 공략 리포트`,
    ]},
    { mechanism:'Specificity', driver:'구체성', variants:[
      (n,v)=>`${n} 3분 안에 파악하는 핵심 방문 요약`,
      (n,v)=>`${n} 3분 요약으로 보는 핵심 내용 축약본`,
      (n,v)=>`${n} 핵심만 3분에 담은 축약 브리핑`,
      (n,v)=>`${n} 바쁜 사람 위한 3분 핵심 압축본`,
    ]},
    { mechanism:'Specificity', driver:'구체성', variants:[
      (n,v)=>`${n} ${v.region}발 동선 포함 루트 + 지도 연결`,
      (n,v)=>`${n} 지도 1클릭 연결 동선 루트 포함 소개`,
      (n,v)=>`${n} 찾아가는 길과 주변 동선 루트 소개`,
      (n,v)=>`${n} ${v.region}에서 찾아가는 루트와 팁 소개`,
    ]},
    ...
    { mechanism:'Contrast', driver:'대비', variants:[
      ...
      (n,v)=>`${n} 입문자 vs 재방문자 코스 차이 한눈에`,
      (n,v)=>`${n} 첫 경험과 재방문 경험의 차이 해부`,
    ]},
    { mechanism:'Contrast', driver:'대비', variants:[
      (n,v)=>`${n} 평일 분위기 vs 주말 분위기 차이 대조`,
      (n,v)=>`${n} 평일과 주말 어떻게 다른지 대조 분석`,
      (n,v)=>`${n} 요일별 분위기 차이 한눈에 대조 보기`,
      (n,v)=>`${n} 주중 vs 주말 분위기 차이점 대조`,
    ]},
    { mechanism:'Contrast', driver:'대비', variants:[
      (n,v)=>`${n} 이른 시간 vs 늦은 시간 장단점 견주기`,
      (n,v)=>`${n} 일찍 가기 vs 늦게 가기 장단점 따져보기`,
      (n,v)=>`${n} 오픈 직후 vs 피크타임 분위기 견주기`,
      (n,v)=>`${n} 시간대별 장단점 꼼꼼히 따져보기`,
    ]},
    { mechanism:'Contrast', driver:'대비', variants:[
      (n,v)=>`${n} 혼자 vs 단체 즐기는 법 달라지는 포인트`,
      (n,v)=>`${n} 1인과 그룹 이용의 차이 핵심 포인트`,
      (n,v)=>`${n} 소규모 모임 vs 대규모 팀 이용 포인트`,
      (n,v)=>`${n} 동행 인원별로 달라지는 이용 포인트`,
    ]},

    // ===== RISK REDUCTION H20-H24 =====
    { mechanism:'RiskReduction', driver:'불안해소', variants:[
      (n,v)=>`${n} 처음이라 걱정될 때 읽으면 좋은 길잡이`,
      (n,v)=>`${n} 첫 방문 불안 줄여주는 입문 길잡이`,
      (n,v)=>`${n} 걱정 없이 출발하는 준비법 길잡이`,
      (n,v)=>`${n} 초행 불안 해소 입문 길잡이 읽어보기`,
    ]},
    { mechanism:'RiskReduction', driver:'불안해소', variants:[
      (n,v)=>`${n} 첫 입장 실수 막아주는 ${v.checklist.length}개 체크`,
      (n,v)=>`${n} 흔한 실수 ${v.checklist.length}가지 미리 막는 체크`,
      (n,v)=>`${n} 미리 체크하면 실수 없는 ${v.checklist.length}가지`,
      (n,v)=>`${n} 실수 줄이는 체크 ${v.checklist.length}개 살피기`,
    ]},
    { mechanism:'RiskReduction', driver:'불안해소', variants:[
      (n,v)=>`${n} 궁금한 점 ${v.faq.length}개 한번에 해결 Q&A`,
      (n,v)=>`${n} 방문 전 ${v.faq.length}개 의문 한번에 풀기`,
      (n,v)=>`${n} 자주 나오는 질문 ${v.faq.length}개 명쾌 답변`,
      (n,v)=>`${n} 알쏭달쏭한 ${v.faq.length}가지 궁금증 깔끔 풀기`,
    ]},
    { mechanism:'RiskReduction', driver:'불안해소', variants:[
      (n,v)=>`${n} 입장부터 귀가까지 흐름 한눈에 흐름도`,
      (n,v)=>`${n} 도착에서 귀가까지 전 과정 순서 흐름도`,
      (n,v)=>`${n} 출발부터 귀가까지 전체 동선 흐름도`,
      (n,v)=>`${n} 시작부터 끝까지 전체 흐름 순서도`,
    ]},
    { mechanism:'RiskReduction', driver:'불안해소', variants:[
      (n,v)=>`${n} 미리 알면 편한 에티켓 + 매너 코드`,
      (n,v)=>`${n} 기본 매너 코드 숙지하고 편하게 즐기기`,
      (n,v)=>`${n} 알아두면 유용한 현장 에티켓 노하우`,
      (n,v)=>`${n} 에티켓 숙지 후 편하게 즐기는 노하우`,
    ]},

    // ===== AUTHORITY H25-H27 =====
    { mechanism:'Authority', driver:'신뢰', variants:[
      (n,v)=>`${n} 편집팀 팩트체크 거친 검증 완료 레퍼런스`,
      (n,v)=>`${n} 편집 원칙에 따라 검증 완료된 레퍼런스`,
      (n,v)=>`${n} 데이터 교차 검증 후 작성한 레퍼런스`,
      (n,v)=>`${n} 팩트체크 거친 운영 실태 검증 레퍼런스`,
    ]},
    { mechanism:'Authority', driver:'신뢰', variants:[
      (n,v)=>`${n} ${UPDATE_DATE} 편집팀 갱신 로그 수록`,
      (n,v)=>`${n} 업데이트 이력 로그 포함 운영 기록 수록`,
      (n,v)=>`${n} 갱신 이력 열람 가능한 운영 기록 수록`,
      (n,v)=>`${n} 변경 로그 포함 운영 기록 열람 가능`,
    ]},
    { mechanism:'Authority', driver:'신뢰', variants:[
      (n,v)=>`${n} 정정 요청 가능한 검증된 열린 자료`,
      (n,v)=>`${n} 오류 제보 채널 포함 검증 자료 열람`,
      (n,v)=>`${n} 열린 정정 절차 포함 검증 자료 열람`,
      (n,v)=>`${n} 수정 접수 가능한 열린 검증 자료`,
    ]},

    // ===== COMBINED H28-H30 =====
    { mechanism:'Combined', driver:'복합', variants:[
      (n,v)=>`${n} 체크 ${v.checklist.length}개 + FAQ ${v.faq.length}개 원스톱 통합`,
      (n,v)=>`${n} 체크리스트와 FAQ 한 곳에 담은 올인원`,
      (n,v)=>`${n} 점검 ${v.checklist.length}항목 + 질문 ${v.faq.length}개 원스톱`,
      (n,v)=>`${n} 입장 준비와 Q&A 한 곳에 묶은 올인원`,
    ]},
    { mechanism:'Combined', driver:'복합', variants:[
      (n,v)=>`${n} ${v.region} 지도 연결 + 체크리스트 올인원`,
      (n,v)=>`${n} 위치 확인 + 준비 항목 한번에 묶은 원스톱`,
      (n,v)=>`${n} 지도 연결 + 체크 ${v.checklist.length}개 한곳에 묶음`,
      (n,v)=>`${n} 네이버지도 + 입장 준비 한곳에 담은 묶음`,
    ]},
    { mechanism:'Combined', driver:'복합', variants:[
      (n,v)=>`${n} 동선·피크타임·FAQ 종합 원스톱 자료`,
      (n,v)=>`${n} 루트 + 시간표 + Q&A 한곳에 묶은 종합`,
      (n,v)=>`${n} 접근 동선·시간대·Q&A 한 곳에 종합`,
      (n,v)=>`${n} 루트와 시간대 FAQ 한곳에 묶은 종합`,
    ]},
  ];
}
