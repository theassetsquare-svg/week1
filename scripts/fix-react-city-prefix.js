...
for (const v of venues) {
  ...
  // React SSR 패턴: "{city}<!-- --> <!-- -->{nameKo}" → "{nameKo}"
  const reactPattern = `${v.city}<!-- --> <!-- -->${v.name}`;
  const count1 = html.split(reactPattern).length - 1;
  if (count1 > 0) html = html.split(reactPattern).join(v.name);

  // React SSR 패턴 변형: "{city}<!-- -->{nameKo}" → "{nameKo}"
  const reactPattern2 = `${v.city}<!-- -->${v.name}`;
  const count2 = html.split(reactPattern2).length - 1;
  if (count2 > 0) html = html.split(reactPattern2).join(v.name);

  // 일반 텍스트: "{city} {nameKo}" → "{nameKo}" (혹시 남은 것)
  const plainPattern = `${v.city} ${v.name}`;
  const count3 = html.split(plainPattern).length - 1;
  if (count3 > 0) html = html.split(plainPattern).join(v.name);

  // JSON 내부: "\"{city} {nameKo}\"" → "\"{nameKo}\""
  // (RSC payload에서 문자열로 저장된 경우)

  const total = count1 + count2 + count3;
  if (total > 0) {
    fs.writeFileSync(htmlPath, html, 'utf8');
    console.log(`[FIXED] ${v.dir}: ${total} occurrences (react:${count1}, react2:${count2}, plain:${count3})`);
    totalFixed += total;
  }
}
...
for (const v of venues) {
  ...
  for (const p of checks) {
    ...
    if (cnt > 0) { console.log(`  [X] ${v.dir}: "${p}" x${cnt}`); remaining += cnt; }
  }
}
if (remaining === 0) console.log('  27개 전체 중복 0건 확인!');
