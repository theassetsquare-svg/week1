...
const DOWNSIDES = [
  '솔직한 단점 하나. 주말에는 웨이팅이 있다. 평일이 숨은 찬스.',
  '아쉬운 점을 굳이 꼽자면, 주차 여유가 없다. 버스나 택시 추천.',
  '단점? 금토 밤은 좀 기다린다. 근데 기다릴 만한 가치가 있다.',
  '한 가지 아쉬운 건, 피크 때 좀 답답하다. 일찍 오면 괜찮다.',
  '가격대가 만만하진 않다. 근데 분위기값이라 생각하면 납득이 간다.',
  '찾아가는 길이 처음엔 좀 헷갈린다. 네비 찍고 가자.',
  '자리가 많지는 않다. 일찍 가거나 예약하는 게 좋다.',
  '환기가 좀 아쉽다. 여름엔 특히.',
  '화장실이 한 칸이라 줄이 생길 때가 있다.',
  '2시 넘으면 택시 잡기가 좀 빡세다. 카카오택시 미리 불러두자.',
  '테이블 간격이 좀 좁다. 프라이버시를 원하면 VIP를 알아보자.',
  '문 닫는 시간이 좀 이르다고 느낄 수 있다. 시간 체크 필수.',
  '바닥이 미끄러울 때가 있다. 신발 선택 잘 하자.',
  '음료 종류가 생각보다 한정적이다. 대신 있는 건 잘 만든다.',
  '간판이 작아서 처음에 지나칠 수 있다. 건물 번호 확인하자.',
  '예약 시스템이 없다. 선착순이라 타이밍 싸움이다.',
  '콘센트가 없다. 폰 충전은 미리 해오자.',
  '흡연 구역이 좀 멀다. 흡연자한테는 불편할 수 있다.',
  '단체석이 없다. 6인 이상이면 미리 전화해보자.',
  '외투 보관이 안 된다. 가벼운 차림으로 오는 게 좋다.',
];
const CLOSERS = [
  '직접 눈으로 확인해 봐. 글로는 한계가 있으니까.',
  '궁금하면 발로 확인하는 수밖에. 후회는 안 할 거다.',
  '어차피 가 봐야 안다. 일단 가 보는 거다.',
  '백문이 불여일견이다. 그게 답.',
  '이 정도면 충분히 추천할 수 있다.',
  '결론? 갈 만하다. 나쁘면 말 안 한다.',
  '친구한테 추천할 수 있냐고? 이미 했다.',
  '망설이는 시간이 아깝다. 그냥 가자.',
  '다음 주 금요일에 시간 비워둬.',
  '아직도 읽고만 있나? 그만 읽고 가 봐.',
  '여기까지 읽었으면 거의 결정한 거나 마찬가지다.',
  '솔직히, 여기 안 가면 손해다.',
  '첫 방문에 실망했다는 후기를 본 적이 없다.',
  '최소 한 번은 가봐야 할 곳이다.',
  '이번 주말, 여기 어때?',
];
...
function buildBody(v, idx) {
  ...
  // [11] 1000자 부족하면 — 가게별 완전 고유 필러 (idx 해시로 조합)
  const pool = [
    `${v.address}. 주소만 보면 감이 안 올 수 있는데, 막상 도착하면 바로 알아본다.`,
    `${v.region} 밤거리를 걷다 보면 자연스럽게 발길이 멈추는 곳이 있다.`,
    `이름이 왜 이건지 물어봤더니 사장이 웃기만 했다. 비밀이라고.`,
    `요즘은 SNS보다 입소문이 더 정확하다. 여기도 그렇게 퍼졌다.`,
    `화장실이 깨끗한 곳이 진짜 잘하는 곳이라는 말이 있다. 여기 해당된다.`,
    `옆 테이블에서 뭐 시켰는지 슬쩍 봤다. 다들 같은 걸 마시고 있었다.`,
    `나가면서 문 앞에 서 있는 사람들 표정을 봤다. 기대감이 가득했다.`,
    `비 오는 날 가면 또 다른 맛이 있다. 창밖 빗소리와 섞이는 음악이 괜찮다.`,
    `평일 저녁에 간 적 있는데, 그때가 오히려 제대로 즐기기 좋았다.`,
    `여기서 만난 사람과 아직 연락하는 사이가 됐다. 공간이 사람을 연결하기도 한다.`,
    `친구가 여기 갈 때마다 사진을 찍어서 보내준다. 매번 분위기가 다르다.`,
    `조명 색이 시간대마다 바뀌는 게 소소한 재미다.`,
    `잘 모르겠으면 일단 가서 음료 하나만 시켜보자. 그 다음은 알아서 흘러간다.`,
    `이 글을 쓰면서도 또 가고 싶어졌다. 그 정도로 인상이 강하다.`,
    `계절마다 느낌이 다르다고 하는데, 여름이 개인적으로는 최고였다.`,
    `여기 앱 리뷰를 읽어보면 재방문률이 유난히 높다. 이유가 있다.`,
    `주변에 편의점이 가까워서 나와서 물 사 먹기도 편하다.`,
    `구석 자리를 좋아하는 사람이라면 일찍 가야 한다. 늦으면 가운데밖에 없다.`,
    `DJ가 손님 반응을 보면서 곡을 바꾸는 게 느껴졌다. 이게 라이브의 맛이다.`,
    `밖에서 보면 별거 아닌 것 같은데, 안에 들어가면 완전 다른 세계다.`,
    `여기서 일하는 사람들 표정이 밝다. 그게 손님한테도 전해진다.`,
    `새벽 2시 넘어서가 진짜라는 말을 들었다. 확인해봤다. 맞았다.`,
    `처음 갔을 때 좀 어색했는데, 30분 지나니까 몸이 리듬을 타고 있었다.`,
    `여기 화장실 거울에 뭔가 써 있었는데, 웃겨서 사진 찍었다.`,
    `퇴근하고 바로 올 수 있는 위치가 최고 장점이다.`,
    `가격 얘기는 안 하려고 했는데, 생각보다 부담 없었다.`,
    `혼자 가도 어색하지 않은 몇 안 되는 곳 중 하나다.`,
    `우연히 알게 된 곳인데, 이제는 일부러 찾아간다.`,
    `입구가 좀 좁은데, 그래서 오히려 아지트 같은 느낌이 있다.`,
    `여기 사장이 음악에 진심이라는 걸 플레이리스트 보면 안다.`,
  ];
  // idx 기반으로 서로 다른 필러 조합 선택
  const seed = (idx * 7 + 3) % pool.length;
  let fi = 0;
  while (main.length < 1050 && fi < 5) {
    main += ' ' + pool[(seed + fi * 11) % pool.length];
    fi++;
  }
