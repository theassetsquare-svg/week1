...
function buildBody(v) {
  ...
  // 1000자 미만이면 보강
  if (body.length < 1000) {
    body += `\n\n${v.region} 지역에서 ${v.typeName} 문화를 이야기할 때 빠지지 않는 이름이다. 첫 방문이라면 피크 시간대를 피해 일찍 도착하는 것을 권한다. 내부 분위기를 충분히 감상하면서 자리를 잡으면, 시간이 지날수록 공간의 진가를 느낄 수 있다. 음악과 조명이 만들어내는 분위기는 직접 경험해 봐야 알 수 있다. 주변 맛집이나 카페와 연계하면 하루 코스로도 알차다.`;
  }
  return body;
}
