...
const hookTemplates = {
  club: [
    v => `${v.region}에서 클럽을 찾고 있다면, 입장료와 시간만 낭비하는 실수는 피해야 합니다. ${v.name_display} 현장 무드를 미리 파악하지 않으면 기대와 다른 밤을 보내기 쉽습니다. 이 가이드에서 방문 전 반드시 알아야 할 정보를 정리했습니다.`,
  ...
  night: [
    v => `${v.region} 나이트에 처음이라면 현장 무드가 낯설 수 있습니다. ${v.name_display}는 어떤 톤인지, 혼자 가도 괜찮은지, 입장 전 무엇을 준비해야 하는지 이 가이드에서 모두 정리했습니다.`,
    v => `${v.name_display} 방문을 앞두고 복장이나 에티켓이 걱정된다면 제대로 찾아왔습니다. 나이트는 클럽과 무드가 다르고, ${v.region} 지역 특유의 문화가 있습니다. 처음 방문자가 자주 실수하는 부분을 미리 정리했습니다.`,
    v => `${v.region}에서 나이트를 찾고 있지만 어디가 나에게 맞는지 모르겠다면, ${v.name_display}부터 확인해 보세요. 현장 톤, 음악 스타일, 시간대별 특징을 한곳에 정리했습니다.`,
    v => `주말 밤 ${v.region}에서 ${v.name_display}를 방문할 계획이라면, 가기 전에 알아두면 좋은 정보가 있습니다. 피크타임, 복장 기준, 안전 수칙까지 한 페이지에 담았습니다.`,
  ],
  lounge: [
    v => `${v.region}에서 무드 좋은 라운지를 찾는다면 ${v.name_display}를 먼저 살펴보세요. 라운지마다 컨셉이 달라서, 모르고 가면 기대와 다를 수 있습니다. 공간 톤, 메뉴 특성, 예약 정보를 미리 정리했습니다.`,
    v => `데이트나 소규모 모임 장소로 ${v.name_display}를 고려 중이라면, 방문 전 이 가이드를 확인하세요. ${v.region} 라운지 중에서도 무드와 가격대가 천차만별이라 사전 정보 파악이 중요합니다.`,
    v => `${v.name_display}에 처음 방문한다면 예약 여부, 드레스코드, 최적 시간대를 먼저 파악하는 것이 좋습니다. ${v.region} 라운지 방문 경험을 한 단계 올려줄 실전 정보를 담았습니다.`,
    v => `${v.region} 라운지 중 ${v.name_display}가 궁금하다면, 무드부터 가격대까지 방문 전 필수 정보를 정리했습니다. 시간대별 톤 변화와 좌석 가이드까지 안내합니다.`,
  ],
};

/* ── Value Promise templates ── */
const vpTemplates = [
  v => `1분만 읽으면 ${v.name_display} 방문에 필요한 체크리스트 ${v.checklist.length}개, 시간대별 흐름 ${v.timeline.length}단계, 자주 묻는 질문 ${v.faq.length}개를 한번에 파악할 수 있습니다.`,
...
function genScanBox(v) {
  ...
  const lines = [
    ...
    `위치: ${addr}`,
    `무드: ${v.bodySections.atmosphere.split('.')[0]}`,
...
const riskPools = {
  club: [
    ...
    { dont: '처음부터 보틀을 주문하지 마세요', doInstead: '먼저 잔 단위로 주문해 현장 무드를 파악한 뒤 결정하세요' },
    { dont: '혼자라고 위축되지 마세요', doInstead: '바 카운터에 자리잡고 음악을 즐기면 자연스럽게 흐름에 녹아듭니다' },
    { dont: '운동화/슬리퍼로 가지 마세요', doInstead: '깔끔한 구두나 부츠가 입장 거부를 방지합니다' },
  ],
  night: [
    { dont: '나이트 무드를 클럽과 같다고 생각하지 마세요', doInstead: '나이트는 파트너 댄스 중심 — 기본 스텝을 미리 알아두면 좋습니다' },
    { dont: '너무 늦은 시간에 도착하지 마세요', doInstead: '나이트는 22~23시가 에너지가 무르익는 시간입니다' },
    ...
    { dont: '첫 방문에 과음하지 마세요', doInstead: '적당히 마시면서 현장 톤을 파악하는 것이 재방문으로 이어집니다' },
  ],
  lounge: [
...
function genTeasers(v) {
  const items = [];
  if (v.timeline.length > 0) items.push(`아래에서 ${v.name_display} 시간대별 흐름 변화 ${v.timeline.length}단계 바로 확인`);
