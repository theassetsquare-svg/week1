...
const CONCLUSION_TEMPLATES = {
  club: (n, r) =>
    `${n}은 ${r}의 밤을 제대로 경험하고 싶은 이들에게 어울리는 선택이다. 타이밍과 복장을 미리 점검하고 방문하면 훨씬 만족스러울 것이다. 이 페이지의 정보는 현장 상황에 따라 달라질 수 있으니 방문 전 재확인을 권장한다. 좋은 밤 되시길.`,
  night: (n, r) =>
    `${n}의 밤은 준비가 된 만큼 더 풍성해진다. 처음이든 재방문이든 ${r}에서 잊기 어려운 하룻밤을 선사하는 곳이다. 정보는 변동될 수 있으니 방문 전 한 번 더 확인하는 것을 권장한다. 좋은 밤 되시길.`,
  lounge: (n, r) =>
    `${n}은 ${r}에서 어른들을 위한 시간을 선사하는 술집이다. 바 한 잔이든 긴 대화든 그 시간을 특별하게 만들어주는 곳이다. 방문 전 정보 변동 여부를 재확인하고 여유로운 저녁을 즐기시길.`,
};
