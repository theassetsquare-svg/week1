...
function generateScenes(v, idx, rng) {
  ...
  // Compositional scenes: 3 fragment pools × 12 variants per scene
  const scenes = {};

  // Scene 1: Arrival
  const s1a = [
    `${timeExpr}, ${name}의 입구에 다다르면 ${adj1} 분위기가 감지된다.`,
    `${name}을 향해 걸음을 옮기니 멀리서부터 ${adj1} 사운드가 새어 나온다.`,
    `${region} 거리 끝자락에서 ${name}의 간판이 눈에 들어왔다.`,
    `${name}의 입구는 ${adj1} 첫인상을 선사한다.`,
    `택시에서 내려 ${name}을 바라보니, ${adj2} 빛이 밤하늘에 스며들어 있다.`,
    `지도 없이도 ${name}은 찾기 쉬웠다. ${adj1} 존재감이 거리에 퍼져 있다.`,
    `${name} 입구에서 기다리는 동안에도 안쪽의 음악이 기대를 높인다.`,
    `${timeExpr}, ${name}에 도착한 순간 ${adj1} 전율이 흐른다.`,
    `${name}의 외관은 ${adj2} 절제미가 돋보인다. 확실한 존재감을 풍긴다.`,
    `약속 장소로 정한 ${name}의 입구에서 설렘이 시작된다.`,
    `내비게이션이 알려준 종착지, ${name}. ${adj1} 인상의 파사드가 기대를 충족시킨다.`,
    `${timeExpr}, ${name} 앞에 선다. 문밖에서도 이곳만의 ${adj2} 공기가 감돈다.`,
  ];
  const s1b = [
    `문을 열고 들어서면 ${adj2} 조명이 시야를 감싼다. ${name} 특유의 분위기가 온몸으로 전해진다.`,
    `입구를 지나자마자 ${name}의 세계가 펼쳐진다. ${adj3} 음향이 공간을 ${verb}.`,
    `${name} 내부로 진입하는 순간, 바깥 세계와의 경계가 ${adj2} 선으로 그어진다.`,
    `안으로 들어서니 ${spaces[0]}이 가장 먼저 눈에 띈다. ${name}의 스케일이 ${adj2} 수준이다.`,
    `입장하니 ${adj3} 사운드가 전신을 감쌌다. ${name}이 기대 이상이라는 걸 직감한다.`,
    `로비를 지나 메인 공간에 이르니, ${name}의 ${adj2} 진면목이 드러난다.`,
    `${name}에 발을 디딘 순간 일상이 뒤로 물러난다. ${adj3} 전환이 시작된다.`,
    `입구의 복도를 지나면 시야가 트인다. ${name}의 메인 홀은 ${adj2} 개방감을 자랑한다.`,
    `안으로 깊이 들어갈수록 ${name}의 ${adj3} 매력이 증폭된다.`,
    `${name} 안쪽은 밖에서 상상한 것과는 ${adj2} 다른 세계다.`,
    `첫 발을 내딛는 순간 ${name}의 ${adj2} 분위기에 빨려 들어간다.`,
    `${name}의 내부는 천장부터 바닥까지 ${adj3} 콘셉트로 통일되어 있다.`,
  ];
  const s1c = [
    `${spaces[0]}을 향해 걸으며 이곳의 에너지를 흡수한다. ${name}에서의 ${adj4} 밤이 본격적으로 시작된다.`,
    `자리를 잡기도 전에 리듬이 귀를 채운다. ${name}의 밤은 이미 ${adj4} 궤도에 올랐다.`,
    `${name}의 동선은 자연스럽게 ${spaces[0]}으로 이끈다. ${adj4} 설계의 결과다.`,
    `${spaces[1]} 근처에서 분위기를 살핀다. ${name}은 ${adj4} 속도로 채워져 간다.`,
    `주변을 둘러보니 저마다의 방식으로 ${name}의 ${adj4} 밤을 즐기고 있다.`,
    `${name}에 오래 머물고 싶다는 생각이 든다. ${adj4} 공간은 시간 감각을 잊게 한다.`,
    `${spaces[0]}에 자리를 잡으니 ${name}의 ${adj4} 전체 풍경이 한눈에 들어온다.`,
    `첫 음료가 나오기 전부터 ${name}의 ${adj4} 분위기만으로 충분히 즐겁다.`,
    `${name}의 밤이 서서히 몸에 스며든다. ${adj4} 특별함이 공기 중에 흐른다.`,
    `함께 온 일행과 눈빛을 교환한다. ${name}이 기대 이상이라는 ${adj4} 무언의 합의.`,
    `${name}과 하나가 되는 느낌을 받으며 ${adj4} 몰입이 시작된다.`,
    `${spaces[1]}에 기대서서 ${name}의 분위기를 천천히 읽는다. ${adj4} 독서 같은 시간이다.`,
  ];
  scenes.scene1 = hPick(s1a, name, 's1a') + ' ' + hPick(s1b, name, 's1b') + ' ' + hPick(s1c, name, 's1c');

  // Scene 2: Interior experience
  const s2a = [
    `${name}의 내부를 탐험하다 보면 ${adj3} 디테일들이 하나씩 드러난다.`,
    `${spaces[1]}에서 음료를 주문하고 관찰하니 ${name}의 ${adj3} 설계 의도가 읽힌다.`,
    `${name}을 깊이 알려면 한 자리에 머무르면 안 된다. 공간마다 ${adj3} 표정이 다르다.`,
    `${adj3} 인테리어가 ${name}의 정체성을 대변한다. 벽면 하나까지 ${adj4} 의미가 있다.`,
    `${name}에서의 시간은 ${adj3} 감각의 축제다. 시각 청각 촉각 모두가 반응한다.`,
    `${spaces[2]}에서 바라본 ${name}의 전경은 ${adj3} 면모를 보인다.`,
    `${name} 곳곳에 숨은 ${adj4} 디자인 요소를 발견하는 재미가 있다.`,
    `${spaces[0]}과 ${spaces[1]} 사이를 오가며 ${name}의 ${adj3} 공간 구조를 파악한다.`,
    `${name}의 조명은 시간대에 따라 미세하게 변한다. ${adj3} 연출력이 돋보인다.`,
    `${name} 내부의 음향은 어디에 앉느냐에 따라 ${adj4} 느낌이 달라진다.`,
    `${spaces[1]}에서 건네받은 음료 한 잔과 함께 ${name}의 ${adj3} 분위기에 젖어든다.`,
    `${adj3} 소재로 마감된 벽면이 ${name}만의 ${adj4} 질감을 만들어낸다.`,
  ];
  const s2b = [
    `${name}의 ${spaces[1]}에서는 대화가 자연스럽게 이어진다. ${adj4} 균형이 절묘하다.`,
    `음료의 첫 모금이 목을 적시면 비로소 ${name}에서의 오늘 밤이 시작되었음을 실감한다.`,
    `${name}에서 눈에 띄는 것은 조명의 층위다. ${adj4} 빛의 그라데이션이 깊이를 더한다.`,
    `${spaces[3]}에서 홀로 음악에 빠진 이도 있고, ${name}의 ${adj4} 분위기를 공유하는 그룹도 있다.`,
    `${name}의 사운드 시스템은 볼륨과 관계없이 ${adj4} 수준을 유지한다.`,
    `이곳의 동선은 ${adj4} 의도로 설계되어 있다. ${name}은 자연스러운 이동을 유도한다.`,
    `${name}의 천장 높이가 만들어내는 개방감이 ${adj4} 방식으로 밤을 즐기게 해준다.`,
    `바텐더의 숙련된 손놀림을 구경하는 것만으로도 ${name}에서의 시간이 ${adj4} 풍요로워진다.`,
    `${spaces[2]}에서 ${spaces[0]}을 바라보면 ${name}의 ${adj4} 에너지 흐름이 한눈에 보인다.`,
    `${name}의 음악 선곡은 시간대별로 달라진다. ${adj4} 초반에서 후반 클라이맥스까지의 설계다.`,
    `테이블 위의 조명, 벽면의 텍스처, 발밑의 진동까지. ${name}은 ${adj4} 오감 경험의 공간이다.`,
    `${adj4} 디테일이 모여 ${name}만의 분위기를 완성한다. 하나하나가 계산된 결과물이다.`,
  ];
  const s2c = [
    `이런 경험을 줄 수 있는 ${typeKr}는 ${region}에서 ${name}이 ${adj1} 유일한 선택일지 모른다.`,
    `${name}에서 보내는 시간은 일상의 리듬과 ${adj1} 다른 속도로 흐른다.`,
    `${name}의 공간 안에서 각자의 밤을 구축하는 사람들의 모습이 ${adj1} 인상적이다.`,
    `${name}을 한마디로 정의하기 어려운 이유는, 경험할 때마다 ${adj1} 다른 면을 보여주기 때문이다.`,
    `${region}의 여러 ${typeKr} 중에서도 ${name}만의 ${adj1} 색채가 분명하다.`,
    `${name}에 머무를수록 이곳이 ${adj1} 유흥 이상의 공간임을 깨닫게 된다.`,
    `매 방문마다 ${adj1} 디테일을 발견하게 되는 것이 ${name}의 깊이다.`,
    `${name}의 매력은 한 번의 방문으로 전부 파악하기 어렵다는 ${adj1} 사실에 있다.`,
    `어느덧 시간이 훌쩍 지나 있다. ${name}의 ${adj1} 몰입감이 강하다는 증거다.`,
    `${name}에서의 경험은 사진으로 담기 어렵다. ${adj1} 직접 와야만 이해되는 공간이다.`,
    `${name}의 모든 요소가 ${adj1} 하나의 방향을 가리킨다. 최고의 밤이라는 목표.`,
    `${name}에서 잔을 비우며 다음 곡을 기다린다. ${adj1} 시간이 아깝지 않은 밤이다.`,
  ];
  scenes.scene2 = hPick(s2a, name, 's2a') + ' ' + hPick(s2b, name, 's2b') + ' ' + hPick(s2c, name, 's2c');

  // Scene 3: Climax
  const s3a = [
    `자정이 가까워지면 ${name}의 에너지는 ${adj1} 정점을 향한다.`,
    `밤이 깊어질수록 ${name}은 가장 ${adj1} 모습을 드러낸다.`,
    `시계를 확인할 필요 없이, ${name}의 ${adj1} 분위기 변화가 시간의 흐름을 알려준다.`,
    `${name}의 클라이맥스는 예고 없이 찾아온다. 에너지가 ${adj1} 한 단계 올라간다.`,
    `새벽으로 향하는 길목에서 ${name}은 가장 ${adj1} 얼굴을 보여준다.`,
    `메인 트랙이 올라오는 순간, ${name} 전체가 ${adj1} 하나의 생명체처럼 움직인다.`,
    `밤의 하이라이트를 향해 ${name}이 가속한다. ${adj1} 공기 자체가 달라진다.`,
    `${name}의 에너지가 ${adj1} 임계점을 넘는 순간이 있다. 그때 모든 것이 달라진다.`,
    `오늘 밤의 피크가 다가온다. ${name}의 공간 전체가 ${adj1} 리듬으로 동기화된다.`,
    `어느 순간 ${name}의 분위기가 확 바뀐다. 가장 ${adj1} 순간의 시작이다.`,
    `클라이맥스 직전의 ${name}은 고요와 폭발 사이의 ${adj1} 긴장감을 품는다.`,
    `${name}에서의 밤이 ${adj1} 정점에 이른다. 이 순간을 위해 여기 온 것이다.`,
  ];
  const s3b = [
    `모든 감각이 열리는 이 순간, ${name}이 줄 수 있는 ${adj3} 최상의 경험이 펼쳐진다.`,
    `${name}의 사운드가 정점에서 폭발한다. ${adj3} 온몸으로 음악을 흡수하는 시간이다.`,
    `${spaces[0]}에서 하나가 된 사람들의 ${adj3} 에너지가 천장을 뚫을 듯하다.`,
    `이 순간 ${name}에 있다는 사실 자체가 축복처럼 느껴진다. ${adj3} 완벽한 밤이다.`,
    `음악이 멈추는 순간조차 환호가 공간을 채운다. ${name}만의 ${adj3} 카타르시스다.`,
    `${name}의 조명이 가장 ${adj3} 패턴으로 전환된다. 시각적 절정이 음악과 동기화된다.`,
    `이 한 순간이 ${name}의 모든 것을 설명한다. ${adj3} 에너지의 정수를 체험하는 시간이다.`,
    `${name}에서 경험하는 이 순간은 일상에서 결코 느낄 수 없는 ${adj3} 강도의 감정이다.`,
    `음악과 조명과 사람들의 에너지가 수렴하는 ${name}의 ${adj3} 마법 같은 시간이다.`,
    `${name}이 보여주는 밤의 절정은 기대를 훨씬 초과한다. ${adj3} 경험이 되어버린다.`,
    `숨을 깊이 들이쉬고 이 순간을 기억에 새긴다. ${name}에서의 ${adj3} 정점이다.`,
    `${name}의 가장 찬란한 시간이 지나간다. 이 ${adj3} 강도의 경험은 쉽게 잊히지 않는다.`,
  ];
  const s3c = [
    `아쉬움이 남는다. ${name}에서의 이 시간이 그만큼 ${adj4} 값졌기 때문이다.`,
    `뒤돌아보면, 아직도 ${name} 안에서는 밤이 계속되고 있다. ${adj4} 다음 방문에 대한 기대가 피어오른다.`,
    `${name}의 여운을 안고 밖으로 나선다. 새벽 공기가 ${adj4} 상쾌하게 느껴진다.`,
    `이 밤의 기억은 오래 남을 것이다. ${name}이 만들어준 ${adj4} 순간들 덕분이다.`,
    `돌아가는 길, ${name}에서의 시간이 벌써 그리워진다. ${adj4} 다음 밤을 기약한다.`,
    `${name}을 나서면서도 귀에는 여전히 마지막 트랙의 ${adj4} 잔향이 남아 있다.`,
    `새벽 거리를 걸으며 ${name}에서의 밤을 복기한다. ${adj4} 완벽한 밤이었다.`,
    `택시 뒷좌석에서 눈을 감는다. ${name}에서의 ${adj4} 밤이 감각의 잔상으로 되살아난다.`,
    `함께한 이들과 자연스럽게 다음 ${name} 방문 약속이 오간다. ${adj4} 자연스러운 귀결이다.`,
    `${name}의 문이 닫히고 밖으로 나오니 ${adj4} 세상이 달라 보인다. 좋은 밤의 후유증이다.`,
    `이 밤을 한 줄로 요약하면: ${name}은 가장 ${adj4} 밤을 만드는 곳이다.`,
    `${name}에서의 경험이 일상으로 돌아간 뒤에도 ${adj4} 자극으로 남아 있다.`,
  ];
  scenes.scene3 = hPick(s3a, name, 's3a') + ' ' + hPick(s3b, name, 's3b') + ' ' + hPick(s3c, name, 's3c');

  // Scene 4: Sensory detail
  const s4a = [
    `${name}에서 가장 먼저 반응하는 감각은 청각이다. ${adj2} 음악의 파동이 피부로 전해진다.`,
    `${name}의 공간을 채우는 것은 음악만이 아니다. 빛, 향, 온도까지 ${adj2} 설계다.`,
    `눈을 감아도 ${name}임을 알 수 있다. 이곳만의 ${adj2} 고유한 사운드 시그니처가 있다.`,
    `${name}의 ${adj2} 감각적 레이어는 겹겹이 쌓여 있다. 하나를 발견하면 또 다른 하나가 보인다.`,
    `${name}의 조명 설계는 장식이 아니다. ${adj2} 감정의 방향을 조절하는 장치다.`,
    `${name}에 들어서면 공기의 질감부터 ${adj2} 다르다. 온도와 습도가 최적으로 유지된다.`,
    `${name}의 음향은 구석구석까지 ${adj2} 균일하게 퍼진다. 어디에 앉든 품질이 일정하다.`,
    `${name}을 감각의 지도로 그려본다면, ${adj2} 사운드가 중심에 있고 빛이 감싸는 구조다.`,
    `바 위에 놓인 음료의 색감마저 ${name}의 ${adj2} 조명 아래에서는 특별해 보인다.`,
    `${name}의 벽면 소재가 만들어내는 음향 반사는 ${adj2} 계산된 결과물이다.`,
    `${spaces[3]}에서 바라본 ${name}의 전경은 ${adj2} 하나의 작품과 같다.`,
    `${name}의 바닥에서 전해지는 미세한 진동까지 ${adj2} 경험의 일부로 설계되어 있다.`,
  ];
  const s4b = [
    `${adj4} 빛의 층위가 ${name}의 공간에 깊이를 더하고, 음악이 귀를 감싼다.`,
    `음료의 첫 모금과 함께 ${name}의 모든 감각적 요소가 하나의 ${adj4} 경험으로 수렴된다.`,
    `시간대에 따라 색온도가 변하며 ${name}의 무드가 ${adj4} 방식으로 전환된다.`,
    `${name}의 향기까지 공간 연출의 일부라는 것을 알아챈 순간, ${adj4} 평가가 달라진다.`,
    `벽면의 텍스처부터 천장 구조물까지 ${name}의 ${adj4} 디자인 요소들이 조화를 이루고 있다.`,
    `${name}의 사운드 엔지니어링은 전문가의 귀에도 ${adj4} 감탄을 자아낼 수준이다.`,
    `빛과 소리의 상호작용이 만들어내는 ${name}만의 ${adj4} 공감각적 경험이 독특하다.`,
    `${adj4} 디테일의 총합이 ${name}만의 고유한 분위기를 만들어낸다.`,
    `잔 위에 맺힌 물방울에도 ${name}의 조명이 반사되어 ${adj4} 작은 우주를 만든다.`,
    `${name}에서의 감각 경험은 시간이 지나도 몸이 기억하는 ${adj4} 종류의 것이다.`,
    `이 공간의 ${adj4} 감각적 밀도는 방문자를 압도하지 않으면서도 깊이를 선사한다.`,
    `${name}의 모든 감각 요소가 동시에 작동할 때, 그것은 ${adj4} 하나의 예술이 된다.`,
  ];
  const s4c = [
    `다른 어떤 공간에서도 느낄 수 없는 ${name}만의 ${adj1} 감각적 시그니처가 여기 있다.`,
    `이런 감각 경험을 선사하는 ${typeKr}는 흔치 않다. ${name}이 ${adj1} 특별한 이유다.`,
    `${name}이 단순한 장소가 아니라 ${adj1} 하나의 경험임을 깨닫게 되는 순간이다.`,
    `감각의 모든 채널이 열린 상태에서 ${name}을 경험하면, ${adj1} 진가를 알 수 있다.`,
    `${name}의 감각적 완성도는 여러 번 방문해야 비로소 ${adj1} 온전히 파악할 수 있다.`,
    `${name}이라는 공간이 감각에 남기는 ${adj1} 각인은 시간이 지나도 선명하다.`,
    `이것이 ${name}만의 방식으로 밤을 정의하는 ${adj1} 감각적 언어다.`,
    `${name}의 감각 설계를 이해하면, 이 공간에 대한 ${adj1} 인식이 완전히 달라진다.`,
    `모든 감각이 동의한다. ${name}은 손꼽히는 ${adj1} ${typeKr}라는 것을.`,
    `${name}을 떠난 후에도 이곳의 ${adj1} 감각적 기억은 오래도록 선명하게 남는다.`,
    `${name}에서의 감각 경험은 다음 방문에 대한 ${adj1} 기대를 자동으로 생성한다.`,
    `이 감각의 향연이 ${name}을 재방문하게 만드는 ${adj1} 핵심 이유다.`,
  ];
  scenes.scene4 = hPick(s4a, name, 's4a') + ' ' + hPick(s4b, name, 's4b') + ' ' + hPick(s4c, name, 's4c');

  // Scene 5: Emotional reflection
  const s5a = [
    `${name}이 밤문화 지형에서 차지하는 위치는 ${adj1} 업소 이상이다.`,
    `돌이켜보면, ${name}에서의 시간은 유흥을 넘어 ${adj1} 하나의 이야기였다.`,
    `${name}을 떠나는 이들에게는 ${adj1} 만족감과 아련한 아쉬움이 공존한다.`,
    `${name}이라는 공간에서 가져가는 것은 사진이나 영수증이 아닌 ${adj1} 감정의 잔향이다.`,
    `${name}에서의 밤을 복기하면, 단순한 시간 소비가 아니었음을 ${adj1} 깨닫게 된다.`,
    `${name}이 주는 감정의 스펙트럼은 생각보다 ${adj1} 넓다. 즐거움 이상의 것이 있다.`,
    `${name}에서 보낸 시간은 일상의 균열을 만들어준다. ${adj1} 좋은 의미에서의 균열이다.`,
    `${name}은 방문자에게 묻는다. 당신에게 ${adj1} 좋은 밤이란 무엇인가.`,
    `${name}에서의 경험은 밤문화에 대한 기존의 편견을 ${adj1} 해체한다.`,
    `${name}이 남기는 여운은 단순한 쾌락의 기억과 ${adj1} 질적으로 다르다.`,
    `${name}에 다녀온 후의 일상이 미묘하게 달라진다. 밤이 가진 ${adj1} 가능성을 알았기 때문이다.`,
    `${name}이라는 경험은 삶의 팔레트에 ${adj1} 새로운 색을 하나 추가해준다.`,
  ];
  const s5b = [
    `이곳은 하루의 무게를 내려놓고 자신만의 시간을 되찾는 ${adj3} 리셋 공간이다.`,
    `밤의 서사 중 한 편을 직접 써내려간 ${adj3} 느낌이다.`,
    `이것이야말로 ${adj3} 좋은 밤문화 공간의 조건이다.`,
    `그것은 다음 방문에 대한 ${adj3} 기대이기도 하다.`,
    `${name}에서의 기억은 시간이 지나도 ${adj3} 여운으로 오래 남는다.`,
    `좋은 밤은 좋은 공간에서 만들어진다는 ${adj3} 진리를 확인시켜준다.`,
    `매 방문마다 조금씩 다른 경험을 안겨주는 것이 ${name}의 ${adj3} 진정한 가치다.`,
    `${name}이 제공하는 것은 서비스가 아니라 하나의 ${adj3} 내러티브다.`,
    `이런 밤이 가능하다는 것을 ${name}을 통해 처음 알게 되는 사람들이 있을 정도로 ${adj3} 경험이다.`,
    `${name}에서의 경험은 축적된다. 방문할수록 ${adj3} 이해가 깊어진다.`,
    `일상 속에서 문득 ${name}의 어떤 순간이 떠오른다. ${adj3} 좋은 밤의 증거다.`,
    `${name}이 만들어주는 ${adj3} 감정의 무게감은 가볍지 않다. 진지한 즐거움이 여기 있다.`,
  ];
  const s5c = [
    `그래서 사람들은 계속해서 ${name}의 문을 ${verb} 두드리게 된다.`,
    `이곳에서 만든 기억은 시간이 지나도 미소를 짓게 만든다. ${name}의 ${verb} 힘이다.`,
    `${name}의 존재는 이 도시의 야간 문화를 더 풍요롭게 ${verb}.`,
    `${name}은 그런 가치를 조용히 전하고 있다. ${verb} 방식으로.`,
    `결국 ${name}이 증명하는 것은, 좋은 밤이 좋은 삶의 일부라는 사실을 ${verb} 것이다.`,
    `${name}에 대한 기대는 방문 후에 더 커진다. ${verb} 진정한 팬심의 시작이다.`,
    `${name}은 밤에 의미를 더하는 공간으로 ${verb} 남아 있다.`,
    `다음 방문을 계획하게 되는 것 자체가 ${name}의 성공을 증명하며 ${verb}.`,
    `${name}이 있어서 이 도시의 밤은 더 다채롭다. ${verb} 부정할 수 없는 사실이다.`,
    `${name}에 대한 이 감정을 한마디로 정리하면: 감사함이 ${verb}.`,
    `${name}이 만들어내는 밤의 서사는 앞으로도 계속 ${verb}.`,
    `${name}과의 다음 만남을 기다리며 일상으로 돌아간다. ${verb} 좋은 밤이었다.`,
  ];
  scenes.scene5 = hPick(s5a, name, 's5a') + ' ' + hPick(s5b, name, 's5b') + ' ' + hPick(s5c, name, 's5c');

  // Scene 6: The People
  const s6a = [
    `${name}에서 눈길을 끄는 것은 공간만이 아니다. 사람들의 ${adj2} 표정이 하나의 풍경이다.`,
    `${name}의 가장 큰 자산은 건물이 아니라 이곳을 찾는 ${adj2} 사람들이다.`,
    `관찰자의 시선으로 ${name}을 바라보면 ${adj2} 흥미로운 패턴이 보인다.`,
    `${name}을 찾는 이들의 공통점은 밤의 시간을 ${adj2} 의미 있게 쓰고 싶은 마음가짐이다.`,
    `${name}의 ${spaces[0]} 주변에는 ${adj2} 다양한 사연의 사람들이 모여든다.`,
    `${name}에 모인 사람들을 관찰하면, 이 도시 밤문화의 ${adj2} 단면이 보인다.`,
    `${name}의 문을 여는 손님 한 명 한 명이 이 공간의 ${adj2} 에너지를 형성한다.`,
    `이른 시간의 ${name}과 깊은 밤의 ${name}은 방문자 구성부터 ${adj2} 다르다.`,
    `${name}의 단골들에게는 ${adj2} 공통된 분위기가 있다. 여유와 취향이 묻어나는 표정이다.`,
    `${name}을 처음 찾는 이와 다섯 번째 찾는 이의 ${adj2} 표정 차이가 흥미롭다.`,
    `${name}에서 혼자 온 사람도, 그룹으로 온 사람도, ${adj2} 각자의 방식으로 이 밤을 즐긴다.`,
    `${name}의 매력은 공간뿐 아니라 이곳에 모인 사람들이 만드는 ${adj2} 에너지에도 있다.`,
  ];
  const s6b = [
    `혼자 음료를 즐기는 이, 웃음을 터뜨리는 그룹이 ${name}에서 ${adj3} 각자의 밤을 보낸다.`,
    `직업도 나이도 다른 사람들이 ${name}의 음악 아래 ${adj3} 하나가 되는 순간이 있다.`,
    `${name}의 스태프도 이 공간의 일부다. ${adj3} 숙련된 손놀림이 방문 경험의 질을 높인다.`,
    `${name}에서 자연스럽게 일어나는 ${adj3} 작은 교류들이 밤의 온도를 올린다.`,
    `서로 모르는 사이에서도 ${name}의 음악에 대한 ${adj3} 공감으로 미소가 교환된다.`,
    `단골과 스태프 사이의 ${adj3} 편안한 인사가 ${name}의 인간적인 분위기를 만든다.`,
    `${name}에서의 사회적 역동은 공간이 유도하는 ${adj3} 자연스러운 흐름이다.`,
    `첫 방문자에게 건네는 ${adj3} 추천 한마디가 ${name}의 호스피탈리티를 보여준다.`,
    `${name}에 모인 사람들의 에너지가 겹치고 ${adj3} 증폭되면서 독특한 분위기가 형성된다.`,
    `이곳에서 만난 인연이 계속 이어지는 경우도 있다. ${name}이 매개가 된 ${adj3} 관계다.`,
    `${name}의 에너지는 사람들의 참여로 완성된다. 모든 방문객이 ${adj3} 주인공이다.`,
    `${name}에서의 인간적인 교류는 이 공간의 숨겨진 ${adj3} 가치 중 하나다.`,
  ];
  const s6c = [
    `밤을 채우는 것은 결국 사람들이며, ${name}은 그 ${adj4} 구심점이다.`,
    `그것이 ${name}의 밤에서 가장 ${adj4} 인간적인 장면이다.`,
    `이런 작은 상호작용들이 모여 ${name}만의 ${adj4} 인간적인 분위기를 만든다.`,
    `덕분에 ${name}의 에너지는 항상 긍정적이고 ${adj4} 자연스러운 교류가 일어난다.`,
    `${name}이 사랑받는 이유 중 하나는 이런 ${adj4} 인간적인 온기 때문이다.`,
    `${name}은 사람과 사람이 만나는 ${adj4} 교차점이다. 밤이 만들어주는 연결이 있다.`,
    `결국 ${name}의 진짜 콘텐츠는 이곳에 모인 사람들 자체인지도 ${adj4} 모른다.`,
    `${name}에서 경험하는 사회적 에너지는 어디서도 복제할 수 없는 ${adj4} 고유한 것이다.`,
    `사람들이 ${name}을 다시 찾는 이유 중 하나는 이 ${adj4} 인간적인 분위기에 있다.`,
    `${name}의 밤은 공간이 만들고, 사람들이 ${adj4} 완성한다.`,
    `${name}에서의 만남은 우연처럼 보이지만, 좋은 공간이 만드는 ${adj4} 필연이기도 하다.`,
    `${name}이 만들어내는 커뮤니티의 ${adj4} 온기는 밤문화의 소중한 자산이다.`,
  ];
  scenes.scene6 = hPick(s6a, name, 's6a') + ' ' + hPick(s6b, name, 's6b') + ' ' + hPick(s6c, name, 's6c');

  // Scene 7: Behind the Scenes
  const s7a = [
    `${name}의 문이 열리기 전부터 ${adj1} 준비는 시작된다. 음향 점검, 조명 세팅, 바 재료 정리.`,
    `보이지 않는 곳에서 ${name}의 ${adj1} 수준이 결정된다.`,
    `${name}이 꾸준히 사랑받는 비결은 ${adj1} 운영의 일관성에 있다.`,
    `${name}의 오프닝 직전은 ${adj1} 전쟁터와 같다. 완벽한 밤을 위한 최종 점검이 진행된다.`,
    `${name}의 ${adj1} 품질은 손님이 도착하기 두 시간 전에 이미 결정된다.`,
    `매일 밤 같은 장소에서 열리지만, ${name}의 밤은 ${adj1} 절대 같지 않다.`,
    `${name}의 무대 뒤에는 방문자가 알지 못하는 ${adj1} 수많은 노력이 숨어 있다.`,
    `${name}이라는 경험의 90%는 손님이 보지 못하는 곳에서 ${adj1} 만들어진다.`,
    `오늘도 ${name}의 팀은 ${adj1} 최고의 밤을 만들기 위한 루틴을 수행한다.`,
    `${name}의 ${adj1} 일관된 품질 뒤에는 체계적인 운영 시스템이 있다.`,
    `${name}의 사운드 엔지니어가 공간의 울림을 계산하는 것에서 ${adj1} 밤이 시작된다.`,
    `${name}의 조명 프로그래밍만 해도 ${adj1} 수십 개의 시나리오가 준비되어 있다.`,
  ];
  const s7b = [
    `음향 엔지니어의 ${adj3} 튜닝, 조명 담당의 색온도 프로그래밍, 바텐더의 메뉴 구성.`,
    `스태프 교육, 장비 관리, 고객 피드백 반영까지 ${adj3} 보이지 않는 노력이 품질을 유지한다.`,
    `매일 밤 같은 수준의 ${adj3} 서비스를 유지하는 것은 생각보다 어려운 일이다.`,
    `방문자에게는 '분위기가 좋다'는 한마디로 압축되지만, 그 뒤에는 ${adj3} 전문가의 노력이 있다.`,
    `요일별, 시즌별로 ${adj3} 미세하게 조정되는 것들이 ${name}의 품질이 된다.`,
    `사운드 체크부터 마감 청소까지, ${name}의 하루는 손님보다 ${adj3} 길다.`,
    `${name}의 바텐더는 매 시즌 ${adj3} 새로운 시그니처 음료를 연구한다.`,
    `${name}의 인테리어도 정기적으로 ${adj3} 소폭 업데이트된다. 변하지 않는 듯하면서 늘 새롭다.`,
    `고객의 동선 데이터를 분석하여 ${adj3} 공간 배치를 최적화하는 것도 ${name} 팀의 업무다.`,
    `${name}의 음악 큐레이션은 최소 일주일 전에 ${adj3} 계획된다. 즉흥이 아닌 설계다.`,
    `방문자 만족도를 추적하고 ${adj3} 개선점을 도출하는 과정이 ${name}의 경쟁력이다.`,
    `${name}의 위생과 안전 관리 기준은 업계 평균을 ${adj3} 상회한다.`,
  ];
  const s7c = [
    `이 모든 준비 과정의 ${adj4} 누적이 곧 ${name}의 품질이 된다.`,
    `${name}이 방문할 때마다 기대를 충족시키는 것은 ${adj4} 결코 우연이 아니다.`,
    `${name}의 모든 경험 뒤에는 이러한 ${adj4} 보이지 않는 헌신이 자리한다.`,
    `이것이 ${name}을 ${adj4} 프로페셔널한 경험의 장으로 만드는 원동력이다.`,
    `${name}의 무대 뒤를 알면, 이곳의 가격이 왜 ${adj4} 가치가 있는지 이해하게 된다.`,
    `${name}의 ${adj4} 진정한 경쟁력은 화려함이 아니라 이런 꾸준함에 있다.`,
    `방문자는 몰라도 된다. 하지만 알게 된다면 ${name}에 대한 ${adj4} 존중이 깊어질 것이다.`,
    `${name}이 오래 사랑받는 이유를 이 ${adj4} 무대 뒤에서 찾을 수 있다.`,
    `다음에 ${name}을 방문하면, 보이지 않는 이 ${adj4} 노력들에 감사하게 될 것이다.`,
    `${name}의 팀에게 매일 밤은 ${adj4} 새로운 공연이다. 그 진정성이 방문자에게 전달된다.`,
    `이런 ${adj4} 프로페셔널리즘이 ${name}을 ${typeKr} 씬에서 돋보이게 만든다.`,
    `${name}의 운영 철학은 ${adj4} 간단하다. 어제보다 나은 오늘 밤을 만드는 것.`,
  ];
  scenes.scene7 = hPick(s7a, name, 's7a') + ' ' + hPick(s7b, name, 's7b') + ' ' + hPick(s7c, name, 's7c');

  // Scene 8: The Return
  const s8a = [
    `${name}에 다시 가게 되는 이유는 사람마다 ${adj2} 다르다.`,
    `처음 방문과 재방문은 ${adj2} 다르다. 처음에는 모든 것이 새롭고 감각이 열려 있다.`,
    `단골이 된다는 것은 그 공간과 ${adj2} 관계를 맺는다는 뜻이다.`,
    `${name}의 재방문율이 높은 데는 ${adj2} 이유가 있다. 같은 공간이지만 매번 다른 밤이다.`,
    `두 번째 ${name} 방문에서 발견한 것은 처음에 놓쳤던 ${adj2} 디테일들이었다.`,
    `${name}을 정기적으로 찾는 이들이 있다. 이들에게 이곳은 ${adj2} 루틴의 일부다.`,
    `한 번의 방문으로 ${name}을 다 안다고 말할 수는 없다. ${adj2} 깊이가 있다.`,
    `${name}에 대한 첫인상과 열 번째 인상은 ${adj2} 상당히 다르다.`,
    `${name}을 세 번 이상 방문한 사람이라면 이 ${adj2} 말에 동의할 것이다.`,
    `시간이 지나면 ${name}이 문득 생각나는 순간이 온다. 그때가 ${adj2} 재방문의 신호다.`,
    `${name}과의 관계는 첫 방문 이후 ${adj2} 본격적으로 시작된다.`,
    `${name}을 다시 찾는 순간, 이전 방문의 ${adj2} 기억이 선명하게 되살아난다.`,
  ];
  const s8b = [
    `누군가는 음악이, 누군가는 조명이, 누군가는 함께한 기억이 ${name}으로 ${adj3} 발걸음을 이끈다.`,
    `재방문에서는 ${name}의 익숙함 속에서 이전에 놓쳤던 ${adj3} 디테일을 발견하게 된다.`,
    `스태프가 얼굴을 알아보고, 선호하는 자리를 기억해주는 ${adj3} 순간이 온다.`,
    `${name}은 방문할수록 ${adj3} 깊이가 느껴지는 공간이다.`,
    `요일에 따라, 동행에 따라 ${name}은 ${adj3} 다른 경험을 제공한다.`,
    `단골들만 아는 ${name}의 숨겨진 ${adj3} 매력 포인트가 하나둘 보이기 시작한다.`,
    `${name}에서 자신만의 ${adj3} 루틴이 생기면, 이곳은 아지트가 된다.`,
    `계절이 바뀌면 ${name}도 ${adj3} 미묘하게 달라진다. 그 변화를 감지하는 것도 단골의 즐거움이다.`,
    `${name}의 새로운 메뉴, 새로운 이벤트가 ${adj3} 재방문의 이유를 더해준다.`,
    `${name}에서의 기억이 켜켜이 쌓이면, 이곳은 ${adj3} 의미 있는 공간으로 변한다.`,
    `${name}을 잘 아는 사람들끼리의 ${adj3} 암묵적인 유대감도 재방문의 매력이다.`,
    `${name}의 변치 않는 핵심과 시즌마다 달라지는 ${adj3} 디테일이 재방문을 흥미롭게 한다.`,
  ];
  const s8c = [
    `이 도시의 밤이 특별한 것은, ${name} 같은 ${adj4} 공간이 존재하기 때문이다.`,
    `${name}은 방문자의 삶에 반복적으로 등장하는 ${adj4} 공간이 되어간다.`,
    `이곳에서의 경험이 단순한 외출 이상이었다는 점이 ${adj4} 재방문의 핵심 동기다.`,
    `${name}의 문을 다시 두드리는 순간, ${adj4} 새로운 밤의 서사가 시작된다.`,
    `세 번째, 네 번째 방문에서는 이미 ${name}이 ${adj4} 자신만의 루틴임을 깨닫게 된다.`,
    `${name}과의 관계는 시간이 갈수록 ${adj4} 깊어진다. 좋은 ${typeKr}의 조건이다.`,
    `${name}을 다시 찾게 되는 것은 선택이 아니라 ${adj4} 자연스러운 귀결이다.`,
    `${name}이라는 이름이 밤과 동의어가 되는 ${adj4} 순간이 온다.`,
    `다음 방문을 이미 기대하고 있다면, ${name}이 ${adj4} 성공한 것이다.`,
    `${name}은 한 번의 방문으로 끝나지 않는다. 이것은 ${adj4} 시작일 뿐이다.`,
    `${name}과 함께 성장하는 밤의 취향이 있다. ${adj4} 그것이 진정한 단골의 여정이다.`,
    `${name}에서의 다음 밤을 기약하며 ${adj4} 오늘의 이야기를 마무리한다.`,
  ];
  scenes.scene8 = hPick(s8a, name, 's8a') + ' ' + hPick(s8b, name, 's8b') + ' ' + hPick(s8c, name, 's8c');
